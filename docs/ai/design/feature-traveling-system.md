...
description: Architecture and technical design for the online tour booking system
---
...
|-------|-----------|--------|
| Frontend | React 18 + Vite | Fast SPA, excellent hot reload |
| State Management | React Context API | Sufficient for current scale |
| Backend | Go 1.21 + Gin | High performance, clean syntax |
| ORM | GORM v2 | Auto-migrate, simple queries |
| Database | MySQL 8.0 | Strong relational model fits the business domain |
| Authentication | JWT (golang-jwt) | Stateless, easy to scale |
| Password | bcrypt | Industry-standard security |
...
| Method | Endpoint | Description | Auth |
|--------|----------|-------------|------|
| GET | `/api/users/:id` | Get personal profile | ✅ JWT |
| PUT | `/api/users/:id` | Update personal profile | ✅ JWT |
...
| POST | `/api/tours` | Create new tour | ✅ Staff |
| PUT | `/api/tours/:id` | Update tour | ✅ Staff |
| DELETE | `/api/tours/:id` | Delete tour | ✅ Staff |

### Tour Schedules

| Method | Endpoint | Description | Auth |
|--------|----------|-------------|------|
| GET | `/api/tours/:id/lich` | List schedules for a tour | ❌ |
| POST | `/api/lichtour` | Create new schedule | ✅ Staff |
| DELETE | `/api/lichtour/:id` | Delete schedule | ✅ Staff |

### Bookings

| Method | Endpoint | Description | Auth |
|--------|----------|-------------|------|
| GET | `/api/pdtour` | All bookings (staff view) | ✅ Staff |
| GET | `/api/pdtour/my` | Current customer's bookings | ✅ Customer |
| POST | `/api/pdtour` | Create new booking | ✅ Customer |

### Invoices

| Method | Endpoint | Description | Auth |
|--------|----------|-------------|------|
| GET | `/api/hoadon` | List all invoices | ✅ Staff |
| POST | `/api/hoadon` | Create invoice from booking | ✅ Staff |
...
// Response 200
{ "success": true, "token": "eyJ...", "user": { "id": 1, "name": "John Doe" } }
...
{
  "success": true,
  "booking": { "id": 10, "ma_pd": "PD202503001", ... }
}
```
...
│   │   └── Register.jsx      ✅ Exists
│   ├── Home/
│   │   └── SearchBar.jsx     ✅ Exists
│   ├── Layout/
│   │   └── Header.jsx        ✅ Exists
│   ├── Profile/
│   │   └── Profile.jsx       ✅ Exists
│   ├── Tour/
│   │   ├── TourList.jsx      🆕 To be created
│   │   ├── TourCard.jsx      🆕 To be created
│   │   └── TourDetail.jsx    🆕 To be created
│   ├── Booking/
│   │   └── BookingForm.jsx   🆕 To be created
│   └── Invoice/
│       └── InvoiceList.jsx   🆕 To be created
├── context/
│   ├── AuthContext.jsx    ✅ Exists
│   └── TourContext.jsx    🆕 To be created
└── pages/
    ├── HomePage.jsx       🆕 To be created
    ├── TourPage.jsx       🆕 To be created
    └── DashboardPage.jsx  🆕 To be created (staff)
```
...
```
server/
├── main.go              ✅ Exists (needs refactor)
├── database/
│   └── database.go      ✅ Exists
├── models/
│   └── models.go        ✅ Exists (needs new models)
├── middleware/
│   └── auth.go          🆕 To be created (JWT middleware)
├── handlers/
│   ├── auth.go          🆕 To be created
│   ├── tour.go          🆕 To be created
│   ├── booking.go       🆕 To be created
│   └── invoice.go       🆕 To be created
└── routes/
    └── routes.go        🆕 To be created
```
...
|----------|--------|--------|
| Auth strategy | JWT (stateless) | Easy to scale, no session store needed |
| Password hashing | bcrypt (cost=10) | Security standard, brute-force resistant |
| API style | RESTful JSON | Simple, easy to test |
| DB migrations | GORM AutoMigrate | Fast for development phase |
| Error format | `{ success, message, data }` | Consistent, easy to handle on client |
| Role-based access | `role` field in ThanhVien (`customer`/`staff`/`admin`) | Simple and sufficient |
...
**Reliability:**
- Database transactions for invoice creation operations
